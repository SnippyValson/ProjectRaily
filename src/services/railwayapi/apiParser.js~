 ...
 */
exports.function getJsonTrainRoute(train_no,eventCallback){
    ...
    http.get(url, function(res) {
        ...
        res.on('end', function () {
            ...
            for (var j=0; j<i; j++){
               ...
               station_string=station_string+station_names[i]+",Arrives at : "+station_arrival[i]+",Departs at :"+station_dep[i]+",on day "+day[i] +".  ";

               }
            var result={speech:station_string,status:station_string};
            eventCallback(result);
      ...
      http.get(url, function(res) {
        ...
        res.on('end', function () {
            ...
            if(status.charAt(0)=='G'&&status.charAt(1)=='N'&&status.charAt(2)=='W')
             ...
             var result={speech:"Sorry, we could not process your request.",status:"Sorry, we could not process your request."};
             eventCallback(result);
    });
}
...
exports.function getJsonTrainBtw(source, dest, date, eventCallback){
        ...
        res.on('end', function () {
            ...
            for (j=0; j<i; j++){
             ...
             var result={speech:train_string,status:train_string};
             eventCallback(result);  
    ...
    http.get(url, function(res) {
        ...
        res.on('end', function () {
           var train_no;
            ...
            var result1={speech:result,status:result};
